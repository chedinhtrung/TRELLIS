...
DONOR_FIT_QUERIES_PER_CATEGORY="${DONOR_FIT_QUERIES_PER_CATEGORY:-48}"
...
POLICY="${POLICY:-$UNIFIED_CALIBRATION_DIR/policy.json}"
if [[ ! -f "$POLICY" && -f "$OUTPUT_DIR/policy.json" ]]; then
...
if [[ ! -f "$RANKINGS" || ( ! -f "$POLICY" && ! -f "$EMBEDDINGS" ) ]]; then
    echo "[0/2] Building the view-18 DINO retrieval index"
    "$PYTHON_BIN" -m interior_reconstruction.retrieval_refinement.dino_index \
...
if [[ -f "$POLICY" ]]; then
    ...
    for path in "$EMBEDDINGS" "$OBJECTIVE1_TRAIN_VOXELS"; do
        if [[ ! -e "$path" ]]; then
    ...
    "$PYTHON_BIN" -m interior_reconstruction.retrieval_refinement.calibrate \
        --train-dir "$TRAIN_DIR" \
        ...
        --donor-fit-queries-per-category "$DONOR_FIT_QUERIES_PER_CATEGORY" \
...
echo "  $OUTPUT_DIR"
echo "Then open notebooks/visualize_results.ipynb and run all cells."
