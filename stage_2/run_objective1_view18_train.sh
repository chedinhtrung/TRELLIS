...
THRESHOLD_DIR="${THRESHOLD_DIR:-$REPO_ROOT/results/ss_coordinate_head_v2/model}"
...
THRESHOLD_FILE="$THRESHOLD_DIR/objective1_threshold.txt"
...
if [[ -z "$COORDINATE_THRESHOLD" ]]; then
    echo "Objective-1 threshold is empty: $THRESHOLD_FILE" >&2
    exit 1
fi
...
for ((gpu = 0; gpu < NUM_GPUS; gpu++)); do
    CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON_BIN" stage_2/export_full_pipeline_voxels.py \
