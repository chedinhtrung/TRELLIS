...
from calibrate_retrieval_v2 import balanced_cap, make_rankings
...
def fit_shared_donor_reranker(path: Path, ridge: float, per_category: int) -> dict:
    """Fit one category-blind donor ranker from the completed v2 table."""
    if not path.is_file():
        raise FileNotFoundError(f"Missing completed donor feature table: {path}")
    with path.open(newline="") as file:
        rows = list(csv.DictReader(file))
    if not rows:
        raise ValueError(f"No donor feature rows in {path}")
    query_categories = {
        row["sample_id"]: row["category"] for row in rows
    }
    by_category: dict[str, list[str]] = defaultdict(list)
    for sample_id, category in query_categories.items():
        by_category[category].append(sample_id)
    selected_by_category = {
        category: sorted(set(sample_ids))[:per_category]
        for category, sample_ids in sorted(by_category.items())
    }
    selected_queries = {
        sample_id
        for sample_ids in selected_by_category.values()
        for sample_id in sample_ids
    }
    selected = [row for row in rows if row["sample_id"] in selected_queries]
    model = fit_query_centered_ridge(
        selected,
        [float(row["target_internal_f1"]) for row in selected],
        [row["sample_id"] for row in selected],
        ridge,
    )
    model["balanced_queries_per_category"] = per_category
    model["training_query_ids"] = sorted(selected_queries)
    model["training_query_counts"] = {
        category: len(sample_ids)
        for category, sample_ids in selected_by_category.items()
    }
    return model
...
def main() -> None:
    ...
    parser.add_argument("--reranker-fit", type=Path, required=True)
    parser.add_argument("--v2-policy", type=Path, required=True)
    parser.add_argument("--v21-policy", type=Path, required=True)
    ...
    parser.add_argument("--donor-reranker-queries-per-category", type=int, default=48)
    ...
    if (
        ...
        or args.donor_reranker_queries_per_category < 1
    ):
    ...
    v2_payload = read_json(args.v2_policy, "retrieval-v2 policy")
    v21_payload = read_json(args.v21_policy, "retrieval-v2.1 policy")
    for payload, label in ((v2_payload, "v2"), (v21_payload, "v2.1")):
        for name, expected in (
            ...
            if int(payload.get(name, -1)) != expected:
                raise ValueError(
                    f"{label} policy {name}={payload.get(name)!r}, expected {expected}"
                )
    operators = extract_champion_operators(v2_payload, v21_payload)
    donor_experts = extract_donor_experts(v21_payload)
    donor_reranker = fit_shared_donor_reranker(
        args.reranker_fit,
        args.donor_ridge,
        args.donor_reranker_queries_per_category,
    )
    print(
    ...
    policy = {
        ...
        "training": {
            ...
            "source_v2_policy": str(args.v2_policy),
            "source_v21_policy": str(args.v21_policy),
        },
    }
