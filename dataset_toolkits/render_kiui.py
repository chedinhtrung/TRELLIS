...
def _load_mesh_with_low_change_multimat_fix(file_path: str) -> Mesh:
    # Fast path: normal kiui loader for single-material-ish assets.
    map_kd_count = _count_map_kd_in_obj(file_path)
    if map_kd_count <= 1:
        return Mesh.load(file_path, resize=False, renormal=True)

    # Low-change fix for multi-material OBJ: bake appearance to vertex color via trimesh.
    # This avoids single-global-albedo collapse without implementing full per-face material rasterization.
...
def _render(file_path, sha256, output_dir, num_views, engine, resolution, samples, denoise, ssaa, shading_mode, ambient_ratio):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--shading_mode", type=str, default="albedo", choices=["lambertian", "albedo", "normal"], help="Shading mode for kiui renderer")
